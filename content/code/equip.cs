...
internal class Equip : UI {
    ...
    internal override float Width => size + TrashPlayer.ChestUpgrade / 2 * size + space * 2;
    ...
    private readonly ItemSlot[] Slots = new ItemSlot[ TrashPlayer.ChestUpgrades ];

    protected override void Initialize() {
        for ( int i = 0; i < TrashPlayer.ChestUpgrades; i++ )
            Slots[ i ] = new( "regularequip" + i ) { Check = () => Main.mouseItem.ModItem is Bauble };
    }

    internal override void Update() {
        for ( int i = 0; i <= TrashPlayer.ChestUpgrade; i++ )
            Slots[ i ].Update();
    }
